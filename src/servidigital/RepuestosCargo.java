 ...
 * @author wiZa
 */
public class RepuestosCargo extends javax.swing.JDialog {
    private int ID;
     ...
     * @param modal
     */
    public RepuestosCargo(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        iniciarTabla();
        try {
            Main.BD.Consultar("select AUTO_INCREMENT from information_schema.TABLES where TABLE_SCHEMA='servidigital' and TABLE_NAME='cargrep'");
            String I = "";
    ...
    private void iniciarTabla() {
        ...
        Object Encabezados[] = new Object[3];
    ...
    private void actualizarCantidades() {
        ...
        for(int i=0; i<Tabla.getRowCount();i++)
            acum += Integer.parseInt(Tabla.getValueAt(i, 2).toString());
    ...
    private void agregarRepuesto() {
        ...
        try {
            if(Repuestos.ID.length() > 0) {
                ...
                if(Main.BD.Siguiente()) {
                    agregarRepuesto(Main.BD.Campo("codigo"),Main.BD.Campo("descrip"));
                }
            }
        }
        catch(SQLException E) {
    ...
    private void procesarCargo() {
        String Descrip  = JOptionPane.showInputDialog(null, "Agregar Nota:", "CARGO", JOptionPane.QUESTION_MESSAGE);
        try {
            Main.BD.Con().setAutoCommit(false);
            Main.BD.Consultar("select AUTO_INCREMENT from information_schema.TABLES where TABLE_SCHEMA='servidigital' and TABLE_NAME='cargrep'");
            Main.BD.Ejecutar("insert into cargrep (usuario,descrip,fecha,deposito) values ("+Login.Actual.getCedula()+",'"+Descrip+"',curdate(),"+D.getId()+")");
            if(Main.BD.Siguiente()) {
                int C = Integer.parseInt(Main.BD.Campo("AUTO_INCREMENT"));
                JOptionPane.showMessageDialog(this, String.valueOf(C));
                for(int i=0; i<Tabla.getRowCount();i++) {
                    Repuesto R = new Repuesto(Tabla.getValueAt(i, 0).toString(), true);
                    Main.BD.Ejecutar("insert into itemcar (cargo,repuesto,cantidad) values ("+C+","+R.getId_repuesto()+","+Tabla.getValueAt(i, 2)+")");
                    Main.BD.Ejecutar("update repuestos set cantidad = cantidad + " + Tabla.getValueAt(i, 2) + " where id_repuesto = " + R.getId_repuesto());
                    Main.BD.Consultar("select * from existencias where repuesto = " + R.getId_repuesto() + " and deposito = " + D.getId());
                    if(Main.BD.Siguiente())
                        Main.BD.Ejecutar("update existencias set cantidad = cantidad + " + Tabla.getValueAt(i, 2) + " where repuesto = " + R.getId_repuesto() + " and deposito = " + D.getId());
                    else
                        Main.BD.Ejecutar("insert into existencias values ("+D.getId()+","+R.getId_repuesto()+","+Tabla.getValueAt(i, 2)+")");
                }
                Main.BD.Con().commit();
                JOptionPane.showMessageDialog(null, "Cargo exitoso","Info",JOptionPane.INFORMATION_MESSAGE);
                Map P = new HashMap();
                P.put("cargo", C);
                new Reporte("/Reportes/cargo.jasper",P).runReporte("Cargo Nº " + C, true);
                this.dispose();
            }
        }
        catch(SQLException e) {
    ...
    private void salir() {
        try {
            ...
            JOptionPane.showMessageDialog(null, Main.BD.showException(e.getMessage()), "CgoRep - Out", JOptionPane.ERROR_MESSAGE);
        }
    }
    /**
    ...
    private void initComponents() {
        ...
        setTitle("CARGO DE INVENTARIO DE REPUESTOS");
        ...
        mnuProcesar.setText("Procesar Cargo");
    ...
    private void tblFilasMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblFilasMouseClicked
        ...
        if(c > 0) tblFilas.setValueAt(c, tblFilas.getSelectedRow(), 2);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(RepuestosCargo.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                RepuestosCargo dialog = new RepuestosCargo(new javax.swing.JFrame(), true);
